...
using RealEstateApp.Core.Application.DTOs.Api.SaleTypes;
    ...
    public class SaleTypeController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                {
                    return NotFound(new { message = "El tipo de venta solicitado no existe." });
                }

                return Ok(new SaleTypeApiDto
                {
        ...
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Create([FromBody] SaveSaleTypeApiRequestDto dto)
        {
            try
            {
                ...
                var result = await _saleTypeService.AddAsync(new SaveSaleTypeDto
                {
                    Name = dto.Name,
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(int id, [FromBody] SaveSaleTypeApiRequestDto dto)
        {
            try
            {
                if (id <= 0)
                ...
                var existing = await _saleTypeService.GetByIdAsync(id);
                if (!existing.IsValid)
                {
                    return StatusCode(StatusCodes.Status500InternalServerError,
                        new { message = "Ocurrió un error interno en el servidor." });
                ...
                var result = await _saleTypeService.UpdateAsync(new SaveSaleTypeDto
                {
                    Id = id,
                    Name = dto.Name,
                    Description = dto.Description
